...
#ifdef CONFIG_USB_AGGR_URB_RX
	atbm_dma_addr_t rx_dma_addr;
	char *rx_dma_addr_buffer;
	unsigned long rx_dma_addr_buffer_len;
#endif
	u8 *pallocated_buf;
};
struct atbm_usb_thread{
#ifdef USB_USE_TASTLET_TXRX
	struct tasklet_struct thread_tasklet;
#else
	struct workqueue_struct *thread_workqueue;
...
struct dvobj_priv{
	...
	atomic_t		  ctrl_err;
#endif
	unsigned long    rx_done_urb_map[BITS_TO_LONGS(RX_URB_NUM)];
...
struct sbus_priv {
	...
	struct atbm_usb_thread	rx_thread;
	struct atbm_usb_thread	tx_thread;
#ifdef USB_USE_TASTLET_TXRX
...
extern struct sk_buff *atbm_get_skb(unsigned int length);
extern bool atbm_rx_serial(struct atbm_common *hw_priv,u8 wsm_seq,int wsm_id);
...
static void atbm_usb_submit_package(struct atbm_common *hw_priv,char *data,int len)
{
	struct sk_buff *atbm_skb_copy;

	atbm_skb_copy = atbm_get_skb(len + 16);
	if (!atbm_skb_copy){
	...
	memmove(atbm_skb_copy->data, data, len);
	atbm_skb_put(atbm_skb_copy,len);
	atbm_skb_copy->pkt_type = ATBM_RX_WSM_DATA_FRAME;
	atbm_rx_single_channel_bh_cb(hw_priv,atbm_skb_copy);
	
}
static int atbm_usb_process_data(struct atbm_common *hw_priv,struct wsm_hdr *wsm)
{
	size_t wsm_len;
	u16 wsm_id;
	u16 msg_id;
	...
	if(atbm_rx_serial(hw_priv,wsm_seq,wsm_id) == false){
		return 0;
	}
	msg_id = wsm_id & (~WSM_TX_LINK_ID(WSM_TX_LINK_ID_MAX));
	atbm_printk_debug("%s:wsm_id(%x),wsm_len(%d) 1\n",__func__,wsm_id,wsm_len);
	switch(msg_id){
	...
	case WSM_SINGLE_CHANNEL_MULTI_RECEIVE_INDICATION_ID:
		{
			...
			struct wsm_multi_rx *  multi_rx = (struct wsm_multi_rx *)wsm;			
			struct wsm_hdr *wsm_child;
			size_t child_len;
			u16 child_id;
			int data_len;
			int RxFrameNum = 0;
			
			RxFrameNum = multi_rx->RxFrameNum;
			data_len = __le16_to_cpu(multi_rx->MsgLen);
			n_frames = RxFrameNum;
			data_len -= sizeof(struct wsm_multi_rx);
			wsm_child = (struct wsm_hdr *)(multi_rx+1);
			child_len = __le16_to_cpu(wsm_child->len);
			child_id	= __le16_to_cpu(wsm_child->id) & 0xFFF;
			do {
						
				if(data_len < child_len){
					atbm_printk_err("skb->len %x,wsm_len %x\n",data_len,child_len);
					break;
				}
				BUG_ON((child_id  & ~WSM_TX_LINK_ID(WSM_TX_LINK_ID_MAX)) !=  WSM_RECEIVE_INDICATION_ID);
				if(child_len > 4096){
					atbm_printk_err("%s:wsm_id[%x],wsm_len[%d]\n",__func__,child_id,child_len);
					BUG_ON(child_len > 4096);
				}
				atbm_usb_submit_package(hw_priv,(char *)wsm_child,child_len);
				
				data_len -= ALIGN(child_len + RX_ALLOC_BUFF_OFFLOAD,4);
				RxFrameNum--;
				wsm_child = (struct wsm_hdr *)((u8 *)wsm_child +ALIGN(( child_len + RX_ALLOC_BUFF_OFFLOAD),4));
				child_len = __le16_to_cpu(wsm_child->len);
				child_id	= __le16_to_cpu(wsm_child->id) & 0xFFF;
	...
	case WSM_SMARTCONFIG_INDICATION_ID:
		atbm_printk_debug("%s:wsm_id(%x),wsm_len(%d) 2\n",__func__,wsm_id,wsm_len);
		if(wsm_len > 4096){
			atbm_printk_err("%s:wsm_id[%x],wsm_len[%d]\n",__func__,wsm_id,wsm_len);
			BUG_ON(wsm_len > 4096);
		}
		atbm_usb_submit_package(hw_priv,(char *)wsm,wsm_len);
		break;
	default:
		/*
		*not need alloc skb
		*/
		if (unlikely(wsm_id == 0x0800)) {
			wsm_handle_exception(hw_priv,(char *)wsm+sizeof(*wsm) ,wsm_len - sizeof(*wsm));
		...
		atbm_printk_debug("%s:wsm_id(%x),wsm_len(%d) 3\n",__func__,wsm_id,wsm_len);
		WARN_ON(wsm_handle_rx(hw_priv, wsm_id, wsm,NULL));
		break;
	}
	return n_frames;
}
static void atbm_usb_rx_tasklet(unsigned long priv)
{
	struct sbus_priv *self = (struct sbus_priv *)(priv);
	struct atbm_common *hw_priv = self->core;
	struct sbus_urb *done_urb = NULL;
	...
	do{
		...
		urb_id = atbm_usb_urb_get(self,self->drvobj->rx_done_urb_map,RX_URB_NUM,0);
		...
		done_urb = &self->drvobj->rx_urb[urb_id];
		recvlength = done_urb->test_urb->actual_length;
		wsm = (struct wsm_hdr *)done_urb->rx_dma_addr_buffer;
		do {
			
			wsm_len = __le16_to_cpu(wsm->len);
			
			if(wsm_len > recvlength){
				atbm_printk_err("%s:wsm_len(%d),recvlength(%d),rev_nums(%d)\n",
					__func__,wsm_len,recvlength,rev_nums);
				break;
			}
			
			rev_nums += atbm_usb_process_data(hw_priv,wsm);
			
			recvlength -= ALIGN(wsm_len,512);
			
			if(recvlength <= 0){
				atbm_bh_multrx_trace(hw_priv,rev_nums);
				atbm_printk_debug("%s:rev_nums(%d),wsm_len(%d)\n",__func__,rev_nums,wsm_len);
				break;
			}
			wsm = (struct wsm_hdr *)((u8 *)wsm + ALIGN(wsm_len,512));
			
		}while(1);

		RTUSB_FILL_RX_BULK_URB(done_urb->test_urb,
			self->drvobj->pusbdev, self->drvobj->ep_in,done_urb->rx_dma_addr_buffer,done_urb->rx_dma_addr_buffer_len,
			atbm_usb_receive_data_complete,done_urb,done_urb->rx_dma_addr);
		status = usb_submit_urb(done_urb->test_urb, GFP_ATOMIC);
		
		if(status < 0){
			WARN_ON(1);
			atbm_usb_urb_put(self,self->drvobj->rx_urb_map,done_urb->urb_id,0);
		}
	}while(1);
}
...
#ifdef USB_USE_TASTLET_TXRX
	atbm_printk_init("atbmwifi USB_USE_TASTLET_TXRX enable (%p)\n",self->core);
	tasklet_init(&self->rx_cmp_tasklet, atbm_usb_rx_tasklet, (unsigned long)self);
#else
	struct atbm_common *hw_priv = self->core;
	atbm_printk_init("atbmwifi INIT_WORK enable\n");
				   ...
				   const void *pdata, int len,sbus_callback_handler hander)
{
	...
	if(urb_id<0){
		...
		atbm_printk_debug("%s:urb_id NULL\n",__func__);
	...
	if(tx_urb->pallocated_buf==NULL)
	{
		...
		atbm_printk_debug("%s:pallocated_buf NULL\n",__func__);
...
#endif
#endif //#ifdef CONFIG_USE_DMA_ADDR_BUFFER
int atbm_usb_urb_malloc(struct sbus_priv *self,struct sbus_urb * pUrb,int max_num)
{
#if 1
	int i=0;
	for(i=0;i<max_num;i++){
...
int atbm_usb_rx_urb_malloc(struct sbus_priv *self)
{
	int ret = 0;
	struct dvobj_priv *pdrvbj = self->drvobj;
	int i = 0;
	
	if(pdrvbj == NULL){
		atbm_printk_err("%s:pdrvbj null\n",__func__);
		return -1;
	}
	
	ret = atbm_usb_urb_malloc(self,pdrvbj->rx_urb,RX_URB_NUM);
	if(ret)
		goto exit;

#ifdef CONFIG_USB_AGGR_URB_RX
	for(i=0;i<RX_URB_NUM;i++){
		pdrvbj->rx_urb[i].rx_dma_addr_buffer_len = 2048*25;
		pdrvbj->rx_urb[i].rx_dma_addr_buffer = atbm_usb_buffer_alloc(self->drvobj->pusbdev,pdrvbj->rx_urb[i].rx_dma_addr_buffer_len, &pdrvbj->rx_urb[i].rx_dma_addr);
	
		if(pdrvbj->rx_urb[i].rx_dma_addr_buffer == NULL){
			int j = i;
			atbm_dbg(ATBM_APOLLO_DBG_ERROR, "Can't allocate dma.");
			for(j = 0;j<i;j++){
				atbm_usb_buffer_free(self->drvobj->pusbdev,pdrvbj->rx_urb[i].rx_dma_addr_buffer_len,pdrvbj->rx_urb[i].rx_dma_addr_buffer, pdrvbj->rx_urb[i].rx_dma_addr);
				pdrvbj->rx_urb[i].rx_dma_addr_buffer = NULL;
			}
			ret = -1;
			goto exit;
		}
...
#else
	for(i=0;i<RX_URB_NUM;i++){
		pdrvbj->rx_urb[i].test_skb = __atbm_dev_alloc_skb(RX_BUFFER_SIZE,GFP_KERNEL);
		if (!pdrvbj->rx_urb[i].test_skb){
			int j = i;
			atbm_dbg(ATBM_APOLLO_DBG_ERROR, "Can't allocate test_skb.");
			for(j = 0;j<i;j++){
				atbm_dev_kfree_skb(pdrvbj->rx_urb[i].test_skb);
				pdrvbj->rx_urb[i].test_skb = NULL;
			}
			ret = -1;
			goto exit;
		}
	...
	return 0;
exit:
	atbm_usb_urb_free(self,pdrvbj->rx_urb,RX_URB_NUM);
	return ret;
}
int atbm_usb_tx_urb_malloc(struct sbus_priv *self)
{
	int ret = 0;
	struct dvobj_priv *pdrvbj = self->drvobj;
	int i = 0;
	...
	ret = atbm_usb_urb_malloc(self,pdrvbj->tx_urb,TX_URB_NUM);
	if(ret)
		goto exit;

	len = TX_BUFFER_SIZE;
#ifdef CONFIG_USE_DMA_ADDR_BUFFER
	len = ALIGN(TX_BUFFER_SIZE,PER_PACKET_LEN);
	self->drvobj->tx_dma_addr = 0;
#ifdef CONFIG_USB_AGGR_URB_TX
	self->drvobj->tx_dma_addr_buffer_len=len*TX_URB_NUM*URB_AGGR_NUM;//ALIGN(len*max_num,4096);
#else
    self->drvobj->tx_dma_addr_buffer_len=len*TX_URB_NUM;//ALIGN(len*max_num,4096);
#endif  //CONFIG_USB_AGGR_URB_TX
	atbm_printk_init("atbm_usb_urb_malloc CONFIG_USE_DMA_ADDR_BUFFER max_num %d, total %d\n",TX_URB_NUM,(int)self->drvobj->tx_dma_addr_buffer_len);
	self->drvobj->tx_dma_addr_buffer = atbm_usb_buffer_alloc(self->drvobj->pusbdev,self->drvobj->tx_dma_addr_buffer_len, &self->drvobj->tx_dma_addr);
	if(self->drvobj->tx_dma_addr_buffer==NULL){
		atbm_dbg(ATBM_APOLLO_DBG_ERROR, "Can't allocate tx_dma_addr_buffer.");
		goto exit;
	...
	for(i= 0;i<TX_URB_NUM;i++){
#ifdef CONFIG_USB_AGGR_URB_TX
		pdrvbj->tx_urb[i].pallocated_buf =NULL;
		pdrvbj->tx_urb[i].test_skb = NULL;
		pdrvbj->tx_urb[i].pallocated_buf_len = 0;
		pdrvbj->tx_urb[i].dma_transfer_addr = 0;
#else
		pdrvbj->tx_urb[i].pallocated_buf =self->drvobj->tx_dma_addr_buffer+i*len;
		pdrvbj->tx_urb[i].test_skb = NULL;
		pdrvbj->tx_urb[i].pallocated_buf_len = len;
		pdrvbj->tx_urb[i].dma_transfer_addr = self->drvobj->tx_dma_addr+i*len;
#endif //USB_ATHENAB_AGGR
	}
#else //#ifdef CONFIG_USE_DMA_ADDR_BUFFER
	for(i= 0;i<TX_URB_NUM;i++){
		pdrvbj->tx_urb[i].pallocated_buf = atbm_kzalloc(len,GFP_KERNEL);
		if(pdrvbj->tx_urb[i].pallocated_buf == NULL){
			int j = i;
			for(j = 0;j<i;j++){
				atbm_kfree(pdrvbj->tx_urb[j].pallocated_buf);
				pdrvbj->tx_urb[j].pallocated_buf = NULL;
			}
	...
	return 0;
exit:
	atbm_usb_urb_free(self,pdrvbj->tx_urb,TX_URB_NUM);
	return ret;
}
int atbm_usb_tx_urb_free(struct sbus_priv *self)
{
	int i =0;
	struct dvobj_priv *pdrvbj = self->drvobj;
	atbm_printk_err("%s:+++\n",__func__);
	...
	if(self->drvobj->tx_dma_addr_buffer){
		atbm_usb_buffer_free(self->drvobj->pusbdev,self->drvobj->tx_dma_addr_buffer_len,self->drvobj->tx_dma_addr_buffer, self->drvobj->tx_dma_addr);
		self->drvobj->tx_dma_addr = 0;
		self->drvobj->tx_dma_addr_buffer = NULL;
	}
#else
	for(i = 0;i<TX_URB_NUM;i++){
		if(pdrvbj->tx_urb[i].pallocated_buf == NULL)
			continue;
		atbm_kfree(pdrvbj->tx_urb[i].pallocated_buf);
		pdrvbj->tx_urb[i].pallocated_buf = NULL;
	}
#endif	
	atbm_printk_err("%s:---\n",__func__);
...
int atbm_usb_rx_urb_free(struct sbus_priv *self)
{
	struct dvobj_priv *pdrvbj = self->drvobj;
	int i = 0;
...
static void atbm_usb_receive_data_complete(struct urb *urb)
{
	...
	struct atbm_common *hw_priv=NULL;
	int recvlength=urb->actual_length;
	char *rx_buff = rx_urb->rx_dma_addr_buffer;
	...
	atbm_usb_urb_put(self,self->drvobj->rx_done_urb_map,rx_urb->urb_id,0);
	atbm_usb_rev_schedule(self);
	return;
resubmit:
	if(!hw_priv->init_done){
...
static int atbm_usb_receive_data(struct sbus_priv *self,unsigned int addr,void *dst, int count,sbus_callback_handler hander)
{
	...
	RTUSB_FILL_RX_BULK_URB(rx_urb->test_urb,
			self->drvobj->pusbdev, self->drvobj->ep_in,rx_urb->rx_dma_addr_buffer,rx_urb->rx_dma_addr_buffer_len,
			atbm_usb_receive_data_complete,rx_urb,rx_urb->rx_dma_addr);
	status = usb_submit_urb(rx_urb->test_urb, GFP_ATOMIC);
	if(status < 0){
		atbm_usb_urb_put(self,self->drvobj->rx_urb_map,urb_id,0);
	}
...
//	status = atbm_usb_urb_malloc(self,dvobj->rx_urb,RX_URB_NUM,RX_BUFFER_SIZE, 1,0);
	status = atbm_usb_rx_urb_malloc(self);
	if (status != 0){
	...
	memset(dvobj->rx_done_urb_map,0xff,sizeof(dvobj->rx_done_urb_map));
	/*3---alloc tx_urb*/
//	status = atbm_usb_urb_malloc(self,dvobj->tx_urb,TX_URB_NUM,TX_BUFFER_SIZE, 0,1);
	status = atbm_usb_tx_urb_malloc(self);
	if (!dvobj->tx_urb){
...
//#define ALIGN(a,b)  (((a)+((b)-1))&(~((b)-1)))
	...
	if (status) {
		...
		atbm_usb_tx_urb_free(self);
		atbm_usb_rx_urb_free(self);
