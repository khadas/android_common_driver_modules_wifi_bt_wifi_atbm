...
static void ieee80211_mgd_enable_netq(struct ieee80211_sub_if_data *sdata,size_t waitting_max,long sleep)
{
	...
	*/
	do{
#if ((defined (AML_KERNEL_VERSION) && AML_KERNEL_VERSION >= 9) || LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0))
		atbm_printk_err("");
#else
...
//#endif
	...
	ie_len = req->ie_len;
#ifdef CONFIG_ATBM_SUPPORT_SAE
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 0, 0)) && (LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0))
			   ...
			   void *cookie)
{
	...
	atbm_printk_mgmt(sdata, "disassociating from %pM by local choice (Reason: %u=%s)\n",
